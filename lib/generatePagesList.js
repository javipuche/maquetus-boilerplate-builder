...
const createLinksObj = (files) => {
    files.forEach((file) => {
        if (file.type === 'directory') {
            ...
            const url = normalize(file.path).split(join(folders.src.pages, '/'))[1].replace(extname(file.path), '.html')
...
const generateIndexPages = () => {
    ...
    hbs.registerHelper('pagesListEq', function (a, b, opts) {
        if (a === b) {
            return opts.fn(this)
        } else {
    ...
    hbs.unregisterHelper('pagesListEq')
    return html
}
